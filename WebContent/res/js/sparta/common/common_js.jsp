...
$j(()=>{
	...
	//핸드폰 국가번호 리스트창 토글
	$j(g_modal).on('click','.css-1rn8qac',e=>{
		const el = $j(e.target).closest('button').next()
		if(el.hasClass('css-1uql5l1')){
			el.attr('class','css-1srbb7s')
		}else{
			el.attr('class','css-1uql5l1')
		}
	})
	// 핸드폰 국가번호 리스트요소 선택
	$j(g_modal).on('click','.css-14plio2 li',e=>{
		$j(e.target).text()
	})
